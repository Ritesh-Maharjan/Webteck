export const servicesData = [
	{
		//Dummy DATA for testing
		//WEB DEVELOPMENT MAIN DATA`
		id: 'web-development',
		serviceNum: 1,
		title: 'Web Development',
		fullDescription:
			'Our web development approach goes beyond looks, we prioritize speed, functionality, SEO optimization, and easy website management afterwards. We assess each project individually and personalize the approach to reach the best result for your specific case.',
		features: [
			'Frontend Web Development',
			'Search Engine Optimization (SEO)',
			'WCAG 2.0 AA Accessible Websites',
			'Responsive Mobile-First Themes',
			'Architecture Planning',
			'Third-Party & Custom API Integrations',
		],
		image: '/img/service-page/web-development.png',

		//SINGLES PAGE DATA
		bgImage: '/img/web-development-detail/webdevelopment-bg.png',
		heading: 'Web Development Service',
		subheading: 'Our extensive knowledge in design and technology brings forth leading brands and digital journeys.',
		infoTitle: 'Our detail-oriented approach to Web development',
		infoDesc:
			'Our web development focuses on more than aesthetics; we prioritize speed, functionality, SEO optimization, and easy post-launch management. Each project is assessed individually, with a tailored approach to deliver the best results for your unique needs.',
		workflowArr: [
			{
				title: 'Environment Setup',
				subtitle: 'We start by configuring environment setup',
				procedures: [
					'Configure development tools',
					'Establish deployment infrastructure',
					'Set up version control systems',
					'Establish Communication Channels',
				],
				ranking: '01',
				img: '/img/home-page/card-1.png',
			},
			{
				title: 'Frontend Construction',
				subtitle: 'Crafting a visually appealing and user-friendly design.',
				procedures: ['Wireframe Development', 'Visual Design Creation', 'Develop responsive layouts', 'Design user interface components'],
				ranking: '02',
				img: '/img/home-page/card-2.png',
			},
			{
				title: 'Testing & Debugging',
				subtitle: 'Turning your design into a fully functional website.',
				procedures: ['Perform manual UX testing', 'Performance Optimization', 'Quality Assurance Testing', 'Back-End Function Intergration'],
				ranking: '03',
				img: '/img/home-page/card-3.png',
			},
			{
				title: 'Performance Optimization',
				subtitle: 'Bringing your vision to the world.',
				procedures: ['Minimize page load times', 'Utilize caching mechanisms', 'Final Pre-Launch Testing', 'Optimize images and assets'],
				ranking: '04',
				img: '/img/home-page/card-4.png',
			},
		],
		detailTitle: 'Web development solutions',
		solutionImage1: '/img/web-development-detail/webdevelopment-img-comp-1.png',
		solutionImage1Title: 'Tailored Web Development',
		solutionImage1Desc:
			'We bring the most amazing designs into customized live web solutions. Through a detailed process, we ensure a seamless user experience, lighting fast speed, everything for a great SEO start, and a web system that any of your colleagues can manage.',
		bulletPoints1: [],
		solutionImage2: '/img/web-development-detail/webdevelopment-img-comp-2.png',
		solutionImage2Title: 'Maintenance services',
		solutionImage2Desc:
			'As your business  grows, maintaining and updating your online presence can become challenging. We offer ongoing support during and after the project to ensure your digital assets stay effective.',
		bulletPoints2: [],
		solutionImage3: '/img/web-development-detail/webdevelopment-img-comp-3.png',
		solutionImage3Title: 'SEO service',
		solutionImage3Desc:
			'Our SEO services are designed to improve your website’s visibility and drive more organic traffic. We start with a comprehensive audit to identify areas for optimization, then implement on-page and technical SEO strategies tailored to your business.',
		bulletPoints3: [],
	},
	{
		//UI/UX design service MAIN DATA
		id: 'ui-ux-design',
		serviceNum: 2,
		title: 'UI/UX Design Service',
		fullDescription:
			"Your website's or product's experience can make or break your results. We make sure you stay ahead of the competition with custom design that is intuitive, modern and resonates with your target audience.",
		features: ['Web Design', 'Wireframe Design', 'Responsive Design', 'User Journey Building', 'Content Architecture', 'A/B testing'],
		image: '/img/service-page/uiux-design.png',

		//SINGLES PAGE DATA
		bgImage: '/img/uiux-detail/uiux-bg.png',
		heading: 'UI/UX design service',
		subheading:
			'From wireframes to polished interfaces, we ensure seamless user experiences, allowing users to navigate effortlessly and enjoy the digital journey',
		infoTitle: 'Our attentive approach to UI/UX design',
		infoDesc:
			'With over 10 years of experience, our professional UI/UX design service goes beyond aesthetics, focusing on user-centred processes. Every design element is crafted to engage your target audience and communicate your unique selling points effectively.',
		workflowArr: [
			{
				title: 'Discovery Stage',
				subtitle: 'We start by configuring environment setup',
				procedures: [
					'Configure development tools',
					'Establish deployment infrastructure',
					'Set up version control systems',
					'Establish Communication Channels',
				],
				ranking: '01',
				img: '/img/home-page/card-1.png',
			},
			{
				title: 'Vision Alignment',
				subtitle: 'Crafting a visually appealing and user-friendly design.',
				procedures: ['Develop responsive layouts', 'Wireframe Development', 'Visual Design Creation', 'Design user interface components'],
				ranking: '02',
				img: '/img/home-page/card-2.png',
			},
			{
				title: 'Wireframes',
				subtitle: 'Turning your design into a fully functional website.',
				procedures: ['Perform manual UX testing', 'Performance Optimization', 'Quality Assurance Testing', 'Back-End Function Intergration'],
				ranking: '03',
				img: '/img/home-page/card-3.png',
			},
			{
				title: 'UI Design',
				subtitle: 'We start by configuring environment setup',
				procedures: [
					'Configure development tools',
					'Establish deployment infrastructure',
					'Set up version control systems',
					'Establish Communication Channels',
				],
				ranking: '04',
				img: '/img/home-page/card-1.png',
			},
			{
				title: 'Evaluation',
				subtitle: 'Crafting a visually appealing and user-friendly design.',
				procedures: ['Develop responsive layouts', 'Wireframe Development', 'Visual Design Creation', 'Design user interface components'],
				ranking: '05',
				img: '/img/home-page/card-2.png',
			},
			{
				title: 'Implementation',
				subtitle: 'Turning your design into a fully functional website.',
				procedures: ['Perform manual UX testing', 'Performance Optimization', 'Quality Assurance Testing', 'Back-End Function Intergration'],
				ranking: '06',
				img: '/img/home-page/card-3.png',
			},
		],
		detailTitle: 'UI/UX design solutions',
		solutionImage1: '/img/uiux-detail/uiux-img-comp-1.png',
		solutionImage1Title: 'UI/UX Design',
		solutionImage1Desc:
			'Transform the way users interact with your brand through thoughtful wireframes and memorable interfaces. Let’s bring your idea to life and create digital experiences that set you apart from the competition',
		bulletPoints1: ['Fast loading times', 'SEO optimization', 'User-friendly management'],
		solutionImage2: '/img/uiux-detail/uiux-img-comp-2.png',
		solutionImage2Title: 'Maintenance services',
		solutionImage2Desc:
			'As your business grows, maintaining and updating your online presence can become challenging. We offer ongoing support during and after the project to ensure your digital assets stay effective.',
		bulletPoints2: ['Fast loading times', 'SEO optimization', 'User-friendly management'],
		solutionImage3: '/img/uiux-detail/uiux-img-comp-3.png',
		solutionImage3Title: 'SEO service',
		solutionImage3Desc:
			'Our SEO services are designed to improve your website’s visibility and drive more organic traffic. We start with a comprehensive audit to identify areas for optimization, then implement on-page and technical SEO strategies tailored to your business.',
		bulletPoints3: ['Fast loading times', 'SEO optimization', 'User-friendly management'],
	},
	{
		//Website Maintenance & Support MAIN DATA
		id: 'website-maintenance',
		serviceNum: 3,
		title: 'Website Maintenance & Support',
		fullDescription:
			"Your website's or product's experience can make or break your results. We make sure you stay ahead of the competition with custom design that is intuitive, modern and resonates with your target audience.",
		features: [
			'Bug Fixes',
			'Updating Contents',
			'Website Migrations',
			'Design & Theme modifications',
			'Content Writing, Consulting & Training',
			'Hosting Provider Assistance',
		],
		image: '/img/service-page/website-maintenance.jpg',

		//SINGLES PAGE DATA
		bgImage: '/img/web-maintenance-detail/webmaintenance-bg.png',
		heading: 'Web Development Service',
		subheading: 'Our extensive knowledge in design and technology brings forth leading brands and digital journeys.',
		infoTitle: 'Our detail-oriented approach to Web development',
		infoDesc:
			'Our web development focuses on more than aesthetics; we prioritize speed, functionality, SEO optimization, and easy post-launch management. Each project is assessed individually, with a tailored approach to deliver the best results for your unique needs.',
		workflowArr: [
			{
				title: 'Environment Setup',
				subtitle: 'We start by configuring environment setup',
				procedures: [
					'Configure development tools',
					'Establish deployment infrastructure',
					'Set up version control systems',
					'Establish Communication Channels',
				],
				ranking: '01',
				img: '/img/home-page/card-1.png',
			},
			{
				title: 'Frontend Construction',
				subtitle: 'Crafting a visually appealing and user-friendly design.',
				procedures: ['Wireframe Development', 'Visual Design Creation', 'Develop responsive layouts', 'Design user interface components'],
				ranking: '02',
				img: '/img/home-page/card-2.png',
			},
			{
				title: 'Testing & Debugging',
				subtitle: 'Turning your design into a fully functional website.',
				procedures: ['Perform manual UX testing', 'Performance Optimization', 'Quality Assurance Testing', 'Back-End Function Intergration'],
				ranking: '03',
				img: '/img/home-page/card-3.png',
			},
			{
				title: 'Performance Optimization',
				subtitle: 'Bringing your vision to the world.',
				procedures: ['Minimize page load times', 'Utilize caching mechanisms', 'Final Pre-Launch Testing', 'Optimize images and assets'],
				ranking: '04',
				img: '/img/home-page/card-4.png',
			},
		],
		detailTitle: 'Web development solutions',
		solutionImage1: '/img/web-maintenance-detail/webmaintanence-img-comp-1.png',
		solutionImage1Title: 'Web Development',
		solutionImage1Desc:
			'We bring the most amazing designs into customized live web solutions. Through a detailed process, we ensure a seamless user experience, lighting fast speed, everything for a great SEO start, and a web system that any of your colleagues can manage.',
		bulletPoints1: [],
		solutionImage2: '/img/web-maintenance-detail/webmaintanence-img-comp-2.png',
		solutionImage2Title: 'Maintenance services',
		solutionImage2Desc:
			'As your business grows, maintaining and updating your online presence can become challenging. We offer ongoing support during and after the project to ensure your digital assets stay effective.',
		bulletPoints2: ['Fast loading times', 'SEO optimization', 'User-friendly management'],
		solutionImage3: '/img/web-maintenance-detail/webmaintanence-img-comp-3.png',
		solutionImage3Title: 'SEO service',
		solutionImage3Desc:
			'Our SEO services are designed to improve your website’s visibility and drive more organic traffic. We start with a comprehensive audit to identify areas for optimization, then implement on-page and technical SEO strategies tailored to your business.',
		bulletPoints3: [],
	},
];

...
export default function ServicePage({ params }: { params: { serviceId: string } }) {
	...
	return (
		<>
			<section
				className={`w-full min-h-[calc(100vh-80px)] px-4 flex flex-col justify-center items-center sm:max-w-screen-sm md:max-w-screen-md lg:max-w-screen-lg xl:max-w-screen-xl 2xl:max-w-screen-2xl mx-auto`}
			>
				{service?.bgImage && (
					<Image
				...
				<div className='flex flex-col items-center gap-5'>
					<HeadingTitle as='h1' className='text-6xl font-bold relative text-center max-w-[800px]'>
						{service?.heading}
					</HeadingTitle>
					<p className='text-center max-w-prose text-lg'>{service?.subheading}</p>
			...
			<section className='gradient-border-service '>
				<h2 className='text-4xl font-bold text-center mb-24'>{service?.detailTitle}</h2>
